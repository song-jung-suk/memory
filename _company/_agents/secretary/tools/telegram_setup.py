...
Secretary 에이전트의 텔레그램 연결 도구. 토큰·chat_id를 Skills의 [설정] 폼에
...
def main():
    if not os.path.exists(CONFIG):
        print("[오류] telegram_setup.json이 없어요. 먼저 [설정] 클릭해서 토큰을 입력해주세요.")
        sys.exit(1)
    try:
    ...
    if not token or not chat:
        print("[오류] TELEGRAM_BOT_TOKEN 또는 TELEGRAM_CHAT_ID가 비어있어요.")
    ...
    try:
        ...
        r.raise_for_status()
        print(f"[정상] 전송 OK - 텔레그램에서 확인하세요. ({len(body)}자)")
    except Exception as e:
        print(f"[오류] 전송 실패: {e}")
